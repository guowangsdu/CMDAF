...
from context_model import rob_d2v_cc_context, rob_d2v_cme_context, rob_hub_cme_context
from model import rob_d2v_cc, rob_d2v_cme
import random
import numpy as np
...
class Trainer():
    def __init__(self, config):
        ...
        self.config = config
        self.criterion = nn.SmoothL1Loss() if config.train_mode == 'regression' else nn.CrossEntropyLoss()
...
def Run(config):
    ...
    if config.dataset_name == 'sims':
        model = rob_hub_cme_context(config).to(device)
