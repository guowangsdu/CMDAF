...
class Dataset_sims(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # load text context
        '''        
        从 index 开始，向前遍历每一个 i，直到达到 self.text_context_length 的限制。
        检查条件 index - i < 0，确保不超出数据集范围；以及 self.video_id[index] != self.video_id[index - i]，确保与当前文本 index 关联的文本与其属于相同的视频。
        如果满足条件，则将 index - i 处的文本转换为字符串，并通过 </s> 连接到 text_context 的前面。
        '''
        text_context = ''
        ...
        # tokenize text+text_context
        context_and_text = text_context + '</s>' + text
        tokenized_context_and_text = self.tokenizer(
            context_and_text,
            max_length=64,
            padding="max_length",  # Pad to the specified max_length.
            truncation=True,  # Truncate to the specified max_length.
        ...
        # extract audio context features
        context_and_audio = torch.cat((audio_context, soundData), 0)
        if len(audio_context) == 0:
            context_and_audio_features = torch.zeros(96000)
            context_and_audio_masks = torch.zeros(96000)
        else:
            features = self.feature_extractor(context_and_audio, sampling_rate=16000, max_length=96000,
                                              return_attention_mask=True, truncation=True, padding="max_length")
            context_and_audio_features = torch.tensor(np.array(features['input_values']), dtype=torch.float32).squeeze()
            context_and_audio_masks = torch.tensor(np.array(features['attention_mask']), dtype=torch.long).squeeze()
        ...
        return {  # text
            ...
            "text_and_context_tokens": torch.tensor(tokenized_context_and_text["input_ids"], dtype=torch.long),
            "text_and_context_masks": torch.tensor(tokenized_context_and_text["attention_mask"], dtype=torch.long),
            # audio
            "audio_inputs": audio_features,
            "audio_masks": audio_masks,
            "audio_context_inputs": audio_context_features,
            "audio_context_masks": audio_context_masks,
            "audio_and_context_inputs": context_and_audio_features,
            "audio_and_context_masks": context_and_audio_masks,
            # labels
            "target": {
                "M": torch.tensor(self.targets_M[index], dtype=torch.float),
                "T": torch.tensor(self.targets_T[index], dtype=torch.float),
                "A": torch.tensor(self.targets_A[index], dtype=torch.float),
            }
        }
...
def collate_fn_sims(batch):   
    ...
    text_context_tokens=[]
    text_context_masks=[]
    text_and_context_tokens=[]
    text_and_context_masks=[]

    audio_inputs = []  
    audio_masks = []
    audio_context_inputs=[]
    audio_context_masks=[]
    audio_and_context_inputs=[]
    ...
    for i in range(len(batch)):
        ...
        text_and_context_tokens.append(batch[i]['text_and_context_tokens'])
        text_and_context_masks.append(batch[i]['text_and_context_masks'])
        #audio
        ...
        audio_and_context_inputs.append(batch[i]['audio_and_context_inputs'])
        audio_and_context_masks.append(batch[i]['audio_and_context_masks'])
    ...
    return {
        # text
            ...
            "text_and_context_tokens": torch.tensor(text_and_context_tokens, dtype=torch.long),
            "text_and_context_masks": torch.tensor(text_and_context_masks, dtype=torch.long),
            # audio
            ...
            "audio_and_context_inputs":  torch.stack(audio_and_context_inputs),
            "audio_and_context_masks":  torch.stack(audio_and_context_masks),
            # # text
            # "text_tokens": torch.tensor(text_tokens, dtype=torch.long),
            # "text_masks": torch.tensor(text_masks, dtype=torch.long),
            # # audio
